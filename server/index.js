...
// 👥 CLIENTES - RUTAS DE GESTIÓN
app.get('/api/clients', async (req, res) => {
  try {
    const clients = await geminiService.clientService.getAllClients()
    res.json({ 
      success: true, 
      clients 
    })
...
app.get('/api/clients/stats', async (req, res) => {
  try {
    const stats = await geminiService.clientService.getStats()
    res.json(stats)
  } catch (error) {
...
app.post('/api/clients/promote', async (req, res) => {
  try {
    ...
    const client = await geminiService.clientService.promoteToVIP(phone)
...
app.post('/api/clients/update', async (req, res) => {
  try {
    ...
    const client = await geminiService.clientService.updateClientName(phone, name)
...
// 🆕 NUEVA RUTA: Actualizar cliente por ID (para compatibilidad con ClientDialog)
...
// 🆕 NUEVA RUTA: Eliminar cliente por ID (para compatibilidad con ClientDialog)
app.delete('/api/clients/:id', async (req, res) => {
  try {
    const { id } = req.params
    
    // Nota: El ClientService actual no tiene función de eliminación
    // Por seguridad, solo retornamos un mensaje informativo
    res.json({ 
      success: false, 
      message: 'La eliminación de clientes no está disponible en el sistema de ChatBot' 
    })
  } catch (error) {
...
// 🆕 NUEVA RUTA: Toggle status de cliente (para compatibilidad con ClientList)
app.patch('/api/clients/:id/toggle', async (req, res) => {
  try {
    const { id } = req.params
    
    // El sistema de ChatBot no usa toggle de status
    // Retornamos un mensaje informativo
    res.json({ 
      success: false, 
      message: 'El toggle de estado no está disponible en el sistema de ChatBot' 
    })
  } catch (error) {
  ...
  try {
    ...
    const processMessage = new Promise(async (resolve, reject) => {
      try {
        ...
        // 👋 DETECTAR SALUDOS Y USAR SISTEMA DINÁMICO
        if (intent === 'greeting') {
          logger.info('👋 Saludo detectado, usando sistema de bienvenida dinámico')
          
          try {
            // Usar el nuevo sistema de saludo dinámico
            const dynamicWelcome = await geminiService.generateWelcomeMessage(from, body)
            
            if (!dynamicWelcome || dynamicWelcome.length < 10) {
              throw new Error('Saludo dinámico vacío o muy corto')
            }
            
            await whatsappService.sendMessage(from, dynamicWelcome)
            ...
            await prisma.conversation.create({
              data: {
                ...
                phone: from,
                message: body,
                response: dynamicWelcome,
                metadata: JSON.stringify({
                  intent: 'greeting',
                  processingMode: 'dynamic_welcome',
            ...
            logger.whatsapp('info', 'Saludo dinámico enviado exitosamente', from, { 
              clientName: client.name,
              duration,
              mode: 'dynamic_welcome',
              messageLength: dynamicWelcome.length
            })
            
            return // 🚫 SALIR TEMPRANO - NO CONTINUAR CON IA
            
          } catch (welcomeError) {
            logger.error('⚠️ Error crítico en saludo dinámico:', welcomeError)
            
            // 🆘 FALLBACK SEGURO - NO CONTINUAR CON IA
            const fallbackMessage = `¡Hola! Soy tu asesor empresarial especializado. ¿En qué puedo ayudarte hoy?`
        ...
        const responses = await geminiService.getResponse(body, knowledgeContext, from)
